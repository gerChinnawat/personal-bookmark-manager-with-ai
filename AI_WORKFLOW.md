...
>
> **Status of this document:** as of 2026-07-28, `pbm-service` has a working NestJS + Prisma
...
> `pbm-ui/` is now a working React + Vite + MUI frontend bound to the real API: Auth0 PKCE
> login/logout, and Collections/Bookmarks/All views wired to their endpoints (see §2 task 14).
> It now also has its own Vitest + React Testing Library suite (see §2 task 15): 21 tests
> covering the pure utils, the service layer, and the Collections/Bookmarks page data-flow
> (load/empty/error/create/optimistic-delete-with-rollback). Sections that depend on actual
> implementation are filled in as that work happens rather than pre-written and later hoped
> to be true; any still marked `[FILL]` below genuinely haven't happened yet (cost
> instrumentation, etc.).
...
| 13 | Add OpenAPI/Swagger docs and example (seed) data | `GET /docs` serves an accurate schema (DTOs annotated, bearer auth scheme registered) without weakening the auth guard; `npm run db:seed` produces 2+ owners' worth of data; both `[FILL]` placeholders this makes fillable (README seed command, this row) closed in the same commit | Done 2026-07-27. `@nestjs/swagger@5.2.1` pinned (last major supporting Nest 8/TS 4.3, same constraint that pinned Prisma in task 9a) with `swagger-ui-express`; `/docs` and `/docs-json` are unauthenticated (dev convenience) but every documented route still requires a real token to call. `prisma/seed.ts` seeds two fabricated owners (`auth0|example-user-1/2`, matching the `auth0|test-user-a/b` shape from the matrix) rather than the real Auth0 test account's `sub`, so a live login still starts empty by design; verified against Postgres directly (4 bookmarks + 2 collections per owner) and against the running server (`/docs-json` lists all 7 routes, `access-token` bearer scheme present). Full `test:security` suite (41 cases) re-run green after the DTO/controller decorator changes |
| 14 | Build `pbm-ui`: Auth0 PKCE login/logout, and Collections/Bookmarks/All screens bound to the real `pbm-service` API | Each feature under `src/features/<feature>/` split into `components/`/`screens/`/`services/`/`interfaces/` (ADR-009); no backend auth endpoint — SPA-only PKCE via `@auth0/auth0-react` (ADR-010); CORS + localStorage token cache so a reload doesn't silently log the user out (ADR-011); `tsc --noEmit` and `eslint .` clean | Done 2026-07-28 across several sessions/commits (`c33acf6` component-to-API binding scaffolding, `b17e306` auth login/logout, `ca55df6` collections, `083f671` bookmarks). Live-testing surfaced two real bugs fixed along the way: the `DELETE` 200-vs-204 mismatch (§9.1) and a CORS preflight 404 that blocked every `:3000→:3001` request until `app.enableCors(...)` was added (ADR-011). Caught afterward, not in this pass: two `react-hooks/set-state-in-effect` lint errors (`BookmarksPage.tsx`, `CollectionsPage.tsx` calling `setState` synchronously at the top of an effect-invoked loader) sat unnoticed because `npm run lint` wasn't run as part of landing the feature commits — found and fixed in a follow-up pass by deferring the loader's initial `setState` calls into a microtask (`Promise.resolve().then(...)`). That same follow-up review also flagged the missing `pbm-ui` test suite, closed in task 15 |
| 15 | Add a `pbm-ui` test suite (Vitest + React Testing Library), scoped to the pure utils, the service layer, and the two data-fetching pages rather than every component | Regression coverage for the loader-effect bug just fixed in task 14 (§9.2), plus the previously-untested service/util layer; `npm run lint`/`tsc -b` stay clean | Done 2026-07-28. `vitest.config.ts` merges `vite.config.ts` via `mergeConfig` (jsdom environment, `@testing-library/jest-dom` setup). 21 tests across 6 files: `utils/url.test.ts`, `utils/relativeTime.test.ts` (fake timers), `collection.service.test.ts`/`bookmark.service.test.ts` (mock `apiService`, assert endpoint/params/envelope-unwrap), `CollectionsPage.test.tsx`/`BookmarksPage.test.tsx` (mock the service layer, assert load/empty/error/create/optimistic-delete-with-rollback via React Testing Library). Deliberately out of scope at the time: `AllPage`, the Auth0 provider/`ProtectedRoute`, layout components, `LoginPage` — a user-instructed scope cut, not an oversight. The auth-adjacent pieces of that cut were revisited in task 16 |
| 16 | Close the `pbm-ui` auth-wiring test gap: agent self-assessed the take-home against the "verification harness" rubric criterion, flagged that `ProtectedRoute` and `api.service.ts` (the client-side route gate and bearer-token/401-handling interceptor — the frontend analogue of the backend's security-critical, 100%-covered pieces) had zero coverage despite being carved out of task 15's scope; user said to add it | `ProtectedRoute` covers the loading/authenticated/redirect-to-`/login` branches; `api.service.ts` covers the request interceptor's token attachment (present/null token) and loading-counter increment, and the response interceptor's loading-counter decrement plus the 401→`onUnauthorized` handler (and non-401 passthrough); `npm run lint`/`tsc -b` stay clean | Done 2026-07-28. `ProtectedRoute.test.tsx` renders through a `MemoryRouter` with `useAuth0` mocked per branch. `api.service.test.ts` mocks the `axios` module itself (not `apiService`) to capture the real request/response interceptor closures registered in the constructor; the loading-counter tests needed `vi.resetModules()` + dynamic re-import per test, since `apiService` is a singleton and its `activeRequests` counter would otherwise leak state across tests. Suite grew from 21 tests/6 files to 34 tests/8 files |
| 17 | User requested a new feature: let a collection be shared with other users via a link, with the ability to enable/disable that link | Owner-only `POST`/`DELETE /collections/:id/share`; a public share-resolve route (`GET /share/collections/:token[/bookmarks]`) as the second `@Public()` exception after `GET /health`; the two `ownerId`-less repository lookups it needs named explicitly as the CLAUDE.md rule-3 exception, not silent; matrix precondition satisfied in the same commit, not a follow-up; docs/README updated in the same pass | Done 2026-07-29. `shareEnabled`/`shareToken` added to `Collection`; token generated once (`crypto.randomBytes(24).toString('base64url')`) and reused across enable/disable toggles rather than rotated, so a link already shared keeps working (ADR-012). `security-matrix.e2e-spec.ts` grew from 41 to 44 cases (19 registered routes, 96 sweep assertions) with a new "collection share links" block covering cross-owner enable/disable 404s and disabled-vs-unknown-token 404 parity; 12 new unit tests for the repository/service methods. `pbm-ui` got a matching `ShareDialog` (toggle + copy link, wired into the previously-stubbed `CollectionCard`/`CollectionsPage` share button) and an unauthenticated `/share/:token` route (`SharedCollectionPage`, reusing `BookmarkCard` read-only) in the same pass, not deferred — verified live in a real browser (not just typechecked): enabled a real seeded collection's link, loaded it unauthenticated in a second tab, confirmed the bookmark and collection name rendered; disabled it and confirmed the same link 404s; re-enabled and confirmed the link reuses the same token per ADR-012. Two `react-hooks/set-state-in-effect` lint errors surfaced while building `ShareDialog`/`SharedCollectionPage` (same failure class as §9.2) and were fixed before landing, not after |
